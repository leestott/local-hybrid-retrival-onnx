...
describe("VectorStore", () => {
  ...
  it("supports semantic search on stored embeddings", () => {
    store.clear();
    store.insert("DOC-010", "Vehicle Checklist", "Operations", 0, "Inspect tires and axle bearings before use.", [1, 0, 0]);
    store.insert("DOC-011", "Gas Leak Detection", "Safety", 0, "Use a gas detector before entering the area.", [0, 1, 0]);

    const results = store.searchSemantic([1, 0, 0], 2);
    assert.equal(results.length, 1);
    assert.equal(results[0].doc_id, "DOC-010");
    assert.equal(results[0].retrievalMode, "semantic");
  });

  it("supports hybrid fusion with lexical and semantic scores", () => {
    store.clear();
    store.insert("DOC-020", "Automotive Guide", "Operations", 0, "Vehicle startup inspection and brake check.", [1, 0, 0]);
    store.insert("DOC-021", "Leak Procedure", "Safety", 0, "Gas leak detection with portable detector.", [0, 1, 0]);

    const results = store.searchHybrid("gas automobile", [1, 0, 0], 2);
    assert.equal(results.length, 2);
    assert.equal(results[0].doc_id, "DOC-020");
