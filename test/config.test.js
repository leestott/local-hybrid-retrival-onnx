...
describe("config", () => {
  ...
  it("has valid RAG settings", () => {
    ...
    assert.ok(config.retrievalModes.includes(config.fallbackRetrievalMode), "fallback mode must be valid");
    assert.ok(config.retrievalModes.includes(config.retrievalMode), "default retrieval mode must be valid");
  });
  ...
  it("has embedding model path configured", () => {
    assert.equal(typeof config.embeddingModelPath, "string");
    assert.ok(config.embeddingModelPath.length > 0, "embeddingModelPath must not be empty");
  });
});
