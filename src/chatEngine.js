...
export class ChatEngine {
  ...
  async init() {
    ...
    this.embeddingEngine = new EmbeddingEngine(config.embeddingModelPath);
    this._emitStatus("embeddings", "Checking local ONNX embedding model...");
    this.semanticAvailable = await this.embeddingEngine.init();

    if (this.semanticAvailable) {
      this._emitStatus(
        "embeddings",
        `Semantic retrieval ready: ${this.store.countEmbeddings()} chunks have ONNX embeddings.`
      );
    } else {
      const { lastError } = this.embeddingEngine.getStatus();
      this._emitStatus(
        "embeddings-fallback",
        `Embedding model unavailable. Using lexical fallback. ${lastError || ""}`.trim()
      );
    }
  ...
  getRetrievalStatus() {
    return {
      configuredMode: config.retrievalMode,
  ...
  async indexDocument(docId, title, category, chunks) {
    const embeddings = this.semanticAvailable
      ? await this.embeddingEngine.embedBatch(chunks)
      : new Array(chunks.length).fill(null);

    for (let i = 0; i < chunks.length; i++) {
      this.store.insert(docId, title, category, i, chunks[i], embeddings[i]);
    }

    return {
      chunks: chunks.length,
      embeddedChunks: embeddings.filter((embedding) => Array.isArray(embedding)).length,
    };
  }
  ...
  async retrieve(query, requestedMode) {
    ...
    const chunks = mode === "semantic"
      ? this.store.searchSemantic(queryEmbedding, topK)
      : this.store.searchHybrid(query, queryEmbedding, topK, config.retrievalWeights);
   ...
   */
  async query(userMessage, history = [], options = {}) {
    // 1. Retrieve relevant chunks
    const { chunks, retrievalMode } = await this.retrieve(userMessage, options.retrievalMode);
   ...
   */
  async *queryStream(userMessage, history = [], options = {}) {
    // 1. Retrieve relevant chunks
    const { chunks, retrievalMode } = await this.retrieve(userMessage, options.retrievalMode);
    const context = this._buildContext(chunks);
