...
app.post("/api/chat", async (req, res) => {
  try {
    const { message, history, compact, retrievalMode } = req.body;
...
app.post("/api/upload", express.raw({ type: "*/*", limit: "2mb" }), async (req, res) => {
  try {
    ...
    const indexed = await engine.indexDocument(docId, title, category, chunks);
...
app.get("/api/health", (_req, res) => {
  res.json({
    status: engineReady ? "ok" : "loading",
    model: config.model,
    retrieval: engine.getRetrievalStatus(),
    ...lastStatus,
  });
});
